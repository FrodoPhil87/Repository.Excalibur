...
class Notification(object):
    def __init__(self, database, addonPath):
        ...
        self.script = os.path.join(self.addonPath, 'MingLee.py')
    ...
    def scheduleNotifications(self):
        xbmc.log("[plugin.program.reloadedtv] Scheduling notifications")
    ...
    def _scheduleNotification(self, channelTitle, programTitle, startTime):
        ...
        xbmc.executebuiltin('AlarmClock(%s-5mins,XBMC.RunScript(%s,%s,%s,%s),%d,True)' %
            (name.encode('utf-8', 'replace'), self.script, channelTitle, programTitle.encode('utf-8', 'replace'), '5min', timeToNotification - 5))

        description = strings(NOTIFICATION_NOW, channelTitle)
        xbmc.executebuiltin('AlarmClock(%s-now,XBMC.RunScript(%s,%s,%s,%s),%d,True)' %
                            (name.encode('utf-8', 'replace'), self.script, channelTitle, programTitle.encode('utf-8', 'replace'), 'now', timeToNotification))
