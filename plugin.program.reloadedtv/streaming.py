...
import urllib
import lolol
import base64 as l1l1l1l1l1l1l1l
...
ADDONSETTING = xbmcaddon.Addon(id='plugin.program.reloadedtv')
dex = 'plugin.video.dex'
reloaded = 'plugin.video.reloaded'
aftermathtv = 'plugin.video.aftermathtv'
ukturk = 'plugin.video.ukturk'
sanctuary = 'plugin.video.sanctuary'
OTTTV     = 'plugin.video.ottalpha'
suicide21 = 'plugin.video.suicidetv21'
freeview  = 'plugin.video.freeview'
ftv = 'plugin.video.F.T.V'
uktvnow = 'plugin.video.uktvnow'
cluiptv = 'plugin.video.cluiptv'
suicide = 'plugin.video.suicidetv'
projectcypher = 'plugin.video.ProjectCypher'
notfilmon = 'plugin.video.notfilmon'
israelive = 'plugin.video.israelive'
adriansports = 'plugin.video.adriansports'
evolve = 'plugin.video.Evolve'


# for unicode utf asci   causes freezing
#import sys
#reload(sys)
#sys.setdefaultencoding("utf-8")


# extras for channel sort and remove wildcards
def read(filename):
    f = file(filename, 'r')
    content = f.read()
    f.close()
...
class StreamsService(object):
    def __init__(self, addon):
        ...
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.program.reloadedtv', 'addons.ini'))
            xbmc.log('[plugin.program.reloadedtv] addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.program.reloadedtv', customFile.split('/')[-1]))
            xbmc.log('[plugin.program.reloadedtv] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)
    ...
    def getIniFiles(self):
        ...
        #files.append(os.path.join(inipath,'plugin.video.dex.ini'))#mayfair: load the generated dex ini I added into add.py

        # meh. Test.  For friends that cant afford subscriptions and struggle with freeIPTV
        #files.append(os.path.join('special://profile', 'addon_data', 'script.renegadestv', 'addons2.ini'))                    
        #files.append(os.path.join('special://profile', 'addon_data', 'script.ivueguide', 'addons.ini'))
        #files.append(os.path.join('special://profile', 'addon_data', 'script.dnatvguide', 'addons.ini'))        
        #files.append(os.path.join('special://profile', 'addon_data', 'script.ufotvguide', 'addons.ini'))
                
        #customFile = str(addon.getSetting('addons.ini.file'))
        #if os.path.exists(customFile):
        #    path = customFile
        #    files.append(os.path.join(customFile))
        '''
        if LOCAL:
    ...
    def detectStream(self, channel):
        ...
        for id in self.getAddons():
            try:
                if id == dex and ADDONSETTING.getSetting('dexter.enabled') == 'false':
                    id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == 'plugin.program.reloadedtv' and ADDONSETTING.getSetting('reloaded.enabled') == 'false':
                    id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == aftermathtv and ADDONSETTING.getSetting('aftermathtv.enabled') == 'false':
                    id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == OTTTV and ADDONSETTING.getSetting('OTTTV.enabled') == 'false':
                    id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == suicide21 and ADDONSETTING.getSetting('suicide21.enabled') == 'false':
                    id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == ukturk and ADDONSETTING.getSetting('ukturk.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == sanctuary and ADDONSETTING.getSetting('sanctuary.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == ftv and ADDONSETTING.getSetting('FTV.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == uktvnow and ADDONSETTING.getSetting('uktvnow.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == cluiptv and ADDONSETTING.getSetting('cluiptv.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == suicide and ADDONSETTING.getSetting('suicide.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == projectcypher and ADDONSETTING.getSetting('projectcypher.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == notfilmon and ADDONSETTING.getSetting('notfilmon.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == israelive and ADDONSETTING.getSetting('israelive.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == adriansports and ADDONSETTING.getSetting('adriansports.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == evolve and ADDONSETTING.getSetting('evolve.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
                if id == freeview and ADDONSETTING.getSetting('freeview.enabled') == 'false':
                    if ADDONSETTING.getSetting('allfreeaddons.enabled') == 'true':
                        pass
                    else:
                        id = 'dummyaddon(to prevent disabled streams from showing)'
            ...
            # Meta and metalq 
            for (label, stream) in self.getAddonStreams(id):
                label, stream = lolol.checktime(label,stream)
                if id == "plugin.video.meta":
                    label = channel.title
                    stream = str(stream.replace("<channel>", channel.title.replace(" ","%20")))
                    
                #'''
                #new way of detecting and listing channels for implemented addons(mayfair)
                if id == dex or id == reloaded or id == OTTTV or id == suicide21 or id == aftermathtv or id == ukturk or id == sanctuary or id == ftv or id == uktvnow or id == cluiptv or id == suicide or id == projectcypher or id == notfilmon or id == israelive or id == adriansports or id == evolve or id == freeview or id == 'plugin.program.reloadedtv':
                    label = label.upper()
                    channel.title = channel.title.upper()
                    if label.startswith(channel.title):
                        if id == 'plugin.program.reloadedtv':
                            stream = stream.replace('reloadedemail@email.com', urllib.quote_plus(ADDONSETTING.getSetting('reloaded.user')))
                            stream = stream.replace('reloadedpassword', urllib.quote_plus(ADDONSETTING.getSetting('reloaded.pass')))
                        ...
                        if id == 'plugin.video.suicidetv21':
                            ...
                            if ADDONSETTING.getSetting('suicide21.format') == 'ts':
                                if not suicide21addon.getSetting('extend') == 'ts':
                                    suicide21addon.setSetting(id = 'extend', value = 'ts')
                                stream = stream.replace('.m3u8','.ts')
                            elif ADDONSETTING.getSetting('suicide21.format') == 'rtmp':
                        ...
                        matches.append((id,label,stream))
                else:#if addon isn't implemented try a different way of matching name to channel
                    label = label.upper()
                    channel.title = channel.title.upper()
