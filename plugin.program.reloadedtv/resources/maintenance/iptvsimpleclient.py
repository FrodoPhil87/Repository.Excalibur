...
def pvrUpdater(updater_path):
    #updater_path = os.path.join(xbmc.translatePath('special://userdata'), 'addon_data/plugin.program.reloadedtv')
    ...
    import xbmcgui
    icon=os.path.join(xbmc.translatePath('special://home'), 'addons/plugin.program.reloadedtv/icon.png')
    dialog = xbmcgui.Dialog();dp = xbmcgui.DialogProgress()
    #xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%("pvr.iptvsimple","M3U and xml added to settings.xml.",3000, icon))
    notify('pvr.iptvsimple','M3U and xml added to settings.xml',os.path.join('special://home/addons', 'plugin.program.reloadedtv', 'icon.png'))##NOTIFY## 
